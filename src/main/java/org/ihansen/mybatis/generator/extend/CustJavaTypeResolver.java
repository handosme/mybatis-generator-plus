...
/**
 * 自动代码生成时oracle数据库字段类型和java类型转处理的自定义实现
 * 
 * @author 吴帅
...
public class CustJavaTypeResolver implements JavaTypeResolver
{
	...
	public FullyQualifiedJavaType calculateJavaType(IntrospectedColumn introspectedColumn)
	{
		...
		{
			// String otherType = jdbcTypeInformation.getJdbcTypeName();
			// if (otherType != null)
			// {
			// if (otherType.equals("TIMESTAMP"))
			// {
			// answer = new FullyQualifiedJavaType(Timestamp.class.getName());
			// }
			// else if (otherType.equals("CHAR"))
			// {
			// answer = new FullyQualifiedJavaType(Character.class.getName());
			// }
			// }
